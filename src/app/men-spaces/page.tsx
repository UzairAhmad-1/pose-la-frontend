...
export default function MenSpacePage() {
  ...
  const handleCardClick = (card) => {
    ...
    switch (card.cardType) {
      ...
      default:
        // For standard cards, navigate directly
        router.push(`/men-spaces/${card.id}`);
    }
  };
  ...
  const handleAccessGhost = (option) => {
    ...
    // "later" does nothing, just closes modal
    // "sensitive" would activate sensitive mode (to be implemented)
  };

  const handleAccessMirror = () => {
    setActiveModal(null);
    if (selectedCard?.mirrorCardId) {
      // Navigate to women's space mirror card
      router.push(`/women-space/${selectedCard.mirrorCardId}`);
    }
  };
        ...
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="flex items-center justify-center gap-4 text-sm flex-wrap">
            <span className="bg-white text-blue-600 px-4 py-2 rounded-full font-semibold shadow-sm">
              {menCards.length} cartes disponibles
            </span>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="mt-16 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-8 border border-blue-100">
          <div className="grid md:grid-cols-3 gap-8 text-center">
            ...
            <div>
              <div className="text-4xl font-bold text-blue-600 mb-2">
                {menCards.filter((c) => !c.locked).length}
              </div>
              <div className="text-sm text-gray-600 font-semibold">
